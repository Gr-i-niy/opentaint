...
- Cap the scan at `--timeout 600` (10 minutes), no longer. If it still errors or times out, don't retry or block — take whatever SARIF was produced and move on; a failed or over-long scan is a coverage gap, not a stopper. In particular, when an OOM or timeout still wrote a well-formed SARIF with results, accept it as-is — don't retry with a higher memory limit; the results are already there
