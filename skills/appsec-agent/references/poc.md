...
After all PoCs, refresh `.opentaint/vulnerabilities.md` yourself (subagents never write it; see SKILL.md) — triage already wrote it from the TP findings (references/triage.md); fold in each TP's PoC outcome now, leading with the dynamically-confirmed ones and marking the rest unconfirmed (a `failed` repro stays a TP, just not dynamically proven).
