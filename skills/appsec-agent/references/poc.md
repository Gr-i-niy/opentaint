...
After all PoCs, refresh `.opentaint/vulnerabilities.md` yourself — triage already wrote it from the TP findings. A dynamic run verified every finding, so collapse to one current-state view: fold in each TP's PoC outcome, dynamically-confirmed first, the rest marked unconfirmed (a `failed` repro stays a TP). Drop any static-run carry-over — nothing stale should remain.
